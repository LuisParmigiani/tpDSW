...
function PerfilUsuario() {
  ...
  useEffect(() => {
    if (!authLoading && usuario) {
      fetchCliente(usuario.id.toString());
    }
  }, [usuario, authLoading]);
  ...
  const onSubmit = async (data: ProfileFormData) => {
    try {
      ...
      const updateData = {
        ...
        ...(updatedImageUrl && { foto: updatedImageUrl }), // Include foto only if it exists
      };
  ...
  if (dataFetched && profileData) {
    return (
      <>
        ...
        <div className="mx-4 sm:max-w-2xl sm:mx-auto bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-8 mt-7 ">
