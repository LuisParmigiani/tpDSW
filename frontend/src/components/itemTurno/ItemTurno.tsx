  ...
  return (
    ...
    > {/* Altura responsive + cursor pointer + hover */}
      <td className="py-2 sm:py-3 px-2 sm:px-4 text-center" onClick={(e) => e.stopPropagation()}>
        <span className="relative flex items-center justify-center">
          ...
          <span className="absolute pointer-events-none inset-0 flex items-center justify-center">
            {selected && (
              <svg className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 20 20" fill="none" style={{ display: 'block' }}>
      ...
      <td className="py-2 sm:py-3 px-2 sm:px-4 text-left">
        <div className="flex items-center gap-1 sm:gap-2 min-w-0"> {/* min-w-0 para permitir truncamiento */}
          <img src={turno.avatar} alt={turno.cliente} className="w-6 h-6 sm:w-8 sm:h-8 rounded-full object-cover flex-shrink-0" />
          <span className="font-medium text-gray-900 truncate max-w-[80px] sm:max-w-[120px] text-xs sm:text-sm" title={turno.cliente}>
            {turno.cliente}
          </span>
        </div>
      </td>
      <td className="py-2 sm:py-3 px-2 sm:px-4 text-gray-900 font-medium text-left hidden sm:table-cell">
        <div className="truncate max-w-[100px] text-xs sm:text-sm" title={turno.fecha}>
          {turno.fecha}
        </div>
      </td>
      <td className="py-2 sm:py-3 px-2 sm:px-4 text-gray-900 font-medium text-left whitespace-nowrap hidden sm:table-cell text-xs sm:text-sm">{turno.hora}</td>
      <td className="py-2 sm:py-3 px-2 sm:px-4 text-gray-900 font-medium text-left">
        <div className="truncate max-w-[100px] sm:max-w-[200px] text-xs sm:text-sm" title={turno.tarea}>
          {turno.tarea}
        </div>
      ...
      <td className="py-2 sm:py-3 px-2 sm:px-4 text-gray-900 font-medium text-left whitespace-nowrap text-xs sm:text-sm">{turno.monto ? `$${turno.monto}` : '-'}</td>
      <td className="py-2 sm:py-3 px-2 sm:px-4 text-left">
        <span
