...
function HorariosSection() {
  ...
  const handleHorarioChange = (index: number, campo: keyof HorarioDia, valor: boolean | string
  ) => {
  ...
  const guardarHorarios = async () => {
    ...
    try {
      ...
      //h es cada dia, 0 a 6 para cada dia de semana
      // si esta activo uso las horas, sino 00:00:00
      const horariosPayload = horarios.map((h, idx) => ({
        diaSemana: idx,
        horaDesde: h.activo ? ( h.horaDesde) : '00:00:00',
        horaHasta: h.activo ? ( h.horaHasta) : '00:00:00',
        usuario: usuario.id,
      }));
